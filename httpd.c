...
//    if (*port == 0)  /* if it needs to allocate a port dynamically */
//    {
//        socklen_t servlen = sizeof(struct sockaddr_in);
//        if (getsockname(listenfd, (struct sockaddr *)&servaddr, &servlen) == -1)
...
int main(void)
{
    ...
    servaddr.sin_family = AF_INET;
    servaddr.sin_addr.s_addr = htonl(INADDR_ANY);
    servaddr.sin_port = htons(5122); /* httpd server */
